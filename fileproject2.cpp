...
class DichVuEmail {
public:
    static void GuiThongBaoDatVe(String^ emailKhachHang, String^ tenKhach, String^ pnr, String^ thongTinChuyenBay, String^ thoiGian, String^ tongTien) {
        try {
            ...
            smtp->Port = 587;
            smtp->EnableSsl = true;
            // Phải tắt DefaultCredentials trước khi gán Credentials mới
            smtp->UseDefaultCredentials = false;
            smtp->DeliveryMethod = SmtpDeliveryMethod::Network;

            // LƯU Ý BẢO MẬT: Mật khẩu này có thể bị Google chặn (Lỗi 5.7.0) nếu chưa phải là "App Password".
            smtp->Credentials = gcnew NetworkCredential("air.connect.notify@gmail.com", "ahph mlqi vimv keob");

            smtp->Send(mail);
        }
        catch (Exception^ ex) {
            MessageBox::Show(L"Hệ thống đặt vé thành công nhưng việc gửi Email thông báo bị lỗi: " + ex->Message + L"\n\nHãy đảm bảo bạn đã dùng Mật khẩu ứng dụng (App Password) 16 ký tự của Google chứ không phải mật khẩu thường.",
